...
pub async fn respond_permission(
    ...
    // Build the response — result.outcome must be an object with { outcome, optionId }
    // The ACP SDK returns `result` from the JSON-RPC response, and the agent checks:
    //   response.outcome?.outcome === "selected"
    //   response.outcome.optionId === "allow" | "allow_always"
    // So the structure must be: result -> outcome -> { outcome: "selected", optionId: "..." }
    let mut outcome_obj = serde_json::json!({
        "outcome": "selected",
        "optionId": option_id
    });
    ...
    if let Some(msg) = user_message {
        if let Some(obj) = outcome_obj.as_object_mut() {
...
/// Save content to a file generated by an agent.
/// Reuses the filesystem path validation to ensure safety.
#[tauri::command(rename_all = "camelCase")]
pub async fn save_generated_file(
    ...
    log::info!("save_generated_file: wrote {} bytes to {}", content.len(), path);
    Ok(())
}
